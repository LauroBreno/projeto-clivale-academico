  ...
  <div class="card-clivale">
    ...
    <div class="stepper-wrapper-outer">
      ...
      <div class="stepper-container">
        <div class="stepper-labels">
          <span :class="{ active: step === 1 }" @click="step = 1"
            >1. IDENTIFICAÇÃO</span
          >
          <span :class="{ active: step === 2 }" @click="validarEtapa1"
            >2. ENDEREÇO</span
          >
          <span :class="{ active: step === 3 }" @click="validarEtapa2"
            >3. FINALIZAÇÃO</span
          >
        </div>
      </div>
    ...
    <form @submit.prevent="finalizar" class="form-content">
        ...
        <div v-show="step === 2" class="inputs-group">
          ...
          <div class="floating-group">
            <input
              type="text"
              :value="
                form.rua && form.bairro ? `${form.rua}, ${form.bairro}` : ''
              "
              placeholder=" "
              disabled
            />
          ...
          <div class="floating-group">
            <input
              type="text"
              v-model="form.numeroComplemento"
              placeholder=" "
            />
      ...
      <div class="footer-actions">
        <button v-if="step < 3" type="button" class="btn-main" @click="proximo">
          PRÓXIMO
        </button>
        <button v-if="step === 3" type="submit" class="btn-main">
          FINALIZAR CADASTRO
        </button>
        <button v-if="step > 1" type="button" class="btn-back" @click="step--">
          VOLTAR
        </button>
      </div>
    </form>

    <p v-if="step === 1" class="toggle-page" @click="$emit('irParaLogin')">
